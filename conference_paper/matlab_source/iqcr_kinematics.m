...
dhT = @(theta,d,a,alpha) [cos(theta) -sin(theta)*cos(alpha)  sin(theta)*sin(alpha) a*cos(theta); ...
                           sin(theta)  cos(theta)*cos(alpha) -cos(theta)*sin(alpha) a*sin(theta); ...
                           0           sin(alpha)             cos(alpha)            d; ...
                           0           0                      0                     1];

function T = fkine(q, a, alpha, d, dhT)
    T = eye(4);
    for i = 1:numel(q)
        T = T * dhT(q(i), d(i), a(i), alpha(i));
    end
end

fk = @(q) fkine(q, a, alpha, d, dhT);
